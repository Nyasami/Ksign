...
struct SourceAppsView: View {
    ...
    private var filteredApps: [(source: ASRepository, app: ASRepository.App)] {
        ...
        case .new:
            result.sort {
                let firstDate =
                    $0.app.currentDate?.date ?? .distantPast

                let secondDate =
                    $1.app.currentDate?.date ?? .distantPast
    ...
    private var storeBanner: some View {
        ZStack(alignment: .bottomTrailing) {
            ...
            VStack(
                ...
                Text(
                    "تصفح التطبيقات والألعاب والأدوات من مكان واحد"
                )
                .font(.subheadline)
                .foregroundStyle(
                    .white.opacity(0.85)
